...
$app->post('/login', function() use ($app) {
    ...
    if ($user != NULL) {
        ...
        $_SESSION['name'] = $user['name'];
            ...
            $response['status'] = "error";
            $response['message'] = 'Ningún usuario está registrado';
        }
...
$app->post('/signUp', function() use ($app) {
    ...
    if(!$isUserExists){
        ...
        if ($result != NULL) {
            $response["status"] = "success";
            $response["message"] = "Cuenta de usuario creada correctamente";
            $response["uid"] = $result;
            if (!isset($_SESSION)) {
                session_start();
            }
            ...
            $response["status"] = "error";
            $response["message"] = "Error al crear el usuario. Por favor, inténtelo de nuevo";
            echoResponse(201, $response);
        }            
    }else{
        $response["status"] = "error";
        $response["message"] = "¡Ya existe un usuario con ese teléfono o correo electrónico!";
        echoResponse(201, $response);
    }
});
